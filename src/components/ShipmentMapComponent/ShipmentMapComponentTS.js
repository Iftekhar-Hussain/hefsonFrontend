// This map component is for trailer and sensor
...
function GoogleMapComponent({ lat, lng, device, reloadMap }) {
  ...
  console.log("tera data ---> ", lat, lng, device, reloadMap);
  const grayscaleFilterStyle = {
        ...
        icon={{
          url:
            device?.realTimeData?.FTemperature1 >
              device?.activeShipment?.temperature?.max ||
            device?.realTimeData?.FTemperature1 <
              device?.activeShipment?.temperature?.min ||
...
function ShipmentMapComponentTS({ lat, lng, device, reloadMap }) {
  return (
...
export default ShipmentMapComponentTS;
